...
def clicked():
    res = "Welcome to " + entry_txt.get()  # Use entry_txt instead of txt
...
entry_txt = Entry(window, width = 0)  # Renamed to entry_txt
...
# -- entry_txt = Entry(window, width=10, state='disabled')
...
# set the checked state (var=chk_state) by passing the checkvalue to the checkbutton
...
# ScrolledText widget
txt_area = scrolledtext.ScrolledText(window, width = 40, height = 10)  # Renamed to txt_area
txt_area.grid(column = 0, row = 6)
# set scrolledtext content
txt_area.insert(INSERT, 'Your text goes here')
...
# specify initial directory for the file dialog by specifying initialdir
# from os import path
